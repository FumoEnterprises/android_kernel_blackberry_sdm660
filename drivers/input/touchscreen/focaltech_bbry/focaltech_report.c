...
struct delayed_work fts_btn_reset_work;
...
static int nav_key_is_reporting;
static int nav_key_need_report;
...
void btn_mask_on_notif(bool btn_mask_on_change)
{
	...
	if (btn_mask_on_change == true) {
		btn_mask_on = true;
		cancel_delayed_work_sync(&fts_btn_reset_work);
		queue_delayed_work(fts_btn_wq, &fts_btn_reset_work, msecs_to_jiffies(5000));
		//INFO_COMMON_FTS("start btn reset work 5000ms");
	} else {
		cancel_delayed_work_sync(&fts_btn_reset_work);
		queue_delayed_work(fts_btn_wq, &fts_btn_reset_work, msecs_to_jiffies(150));
...
int btn_mask_function_init(void)
{
	...
	INIT_DELAYED_WORK(&fts_btn_reset_work, btn_reset_work);
...
void fts_navkey_ignore(void)
{
	if (nav_key_is_reporting)
		nav_key_need_report = 0;
}

EXPORT_SYMBOL(fts_navkey_ignore);

EXPORT_SYMBOL(btn_mask_on_notif);
#endif
